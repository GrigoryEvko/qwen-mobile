...
declare -A SWITCH=(
    ...
    [vocab-dup-tokens]=FUZZ_MODEL_LOAD_KNOWN_DUP_TOKENS
    [spm-byte-tokens]=FUZZ_MODEL_LOAD_KNOWN_SPM_BYTES
    [vocab-none-assert]=FUZZ_MODEL_LOAD_KNOWN_NONE_VOCAB
    [vocab-byte-type]=FUZZ_MODEL_LOAD_KNOWN_BYTE_TYPE
...
declare -A EXPECT=(
    ...
    [vocab-none-assert]='GGML_ASSERT\(pimpl->type != LLAMA_VOCAB_TYPE_NONE\)'
    [tokenizer-codepoint]='invalid codepoint'
    [jinja-float-cast]='jinja/value\.h:[0-9]+:[0-9]+: runtime error: .*outside the range of representable values'
    [jinja-parser-assert]='jinja::chk_type'
